Теперь у меня полная картина возможностей Gemini CLI: встроенный `google_web_search`, `web_fetch`, MCP-серверы . Пишу полностью модернизированный агент.
...
```json
{
  ...
  "annotation": "Фундаментальный учебник по синоптической метеорологии,
    широко используемый в российских гидрометеорологических вузах.
    Глава 3 полностью посвящена барической топографии — дано строгое
    физическое обоснование, приведены все ключевые формулы и примеры
    анализа реальных карт.",
  "citation_gost": "Хромов С.П., Петросянц М.А. Метеорология и климатология :
    учебник / С.П. Хромов, М.А. Петросянц. — 8-е изд. — М. :
    Издательство МГУ, 2012. — 582 с.",
  "notes": "Противоречие с ref_005 в части определения ОТ — см. analysis_notes.md"
}
```
...
- Предпочитай отечественную терминологию (если вопрос не о зарубежных стандартах)
...
> [ref_003, с. 13]: формула гипсометрического уравнения

### Противоречия
- Обозначение средней температуры: T̄ (ref_001) vs T_ср (ref_003) → унифицировать как T̄
...
"Совокупность изобарических поверхностей, положение которых в пространстве
определяется значениями геопотенциала на данном уровне."

**Уточнение** (ref_003, с. 12): [если отличается]
...
|------|---------|---------|--------|
| hromov_meteorology.pdf | 582 | 1,2,3,4 | 0.95 |

### Веб-поиск выполнен
| Запрос | Результатов | Скачано | Использовано |
|--------|-------------|---------|--------------|
| "барическая топография" учебник | 23 | 5 | 3 |

### Источники НЕДОСТУПНЫ (требуют авторизации)
- elibrary.ru/item.asp?id=xxxxx → только аннотация, нет полного текста
- springer.com/... → платный доступ
...
## Конфигурация инструментов Gemini CLI

Убедись, что в `~/.gemini/settings.json` НЕ отключён веб-поиск:
```json
{
  "tools": {
    "exclude": []
...
Используемые встроенные инструменты:
- `google_web_search` — поиск в интернете с grounding [cite:web:87]
- `web_fetch` — скачивание страниц и файлов [cite:web:85]
